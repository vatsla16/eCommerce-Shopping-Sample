...
(function($){
	...
	$(document).ready(function($){
		...
		function getPaginationPages(e) {
			...
			var id = $this.parents('.themify_lightbox_visibility').data('item');
			var selected = $('#setting-hooks-' + id + '-visibility').val();
			$.ajax({
				url: ajaxurl,
				type: 'post',
				data: {
		...
		$body.on('click', '.visibility-save', function(){
		...
		function import_assets( res, callback ) {
			var queue = [];
			var max_query = 5; // maximum number of requests to send simultaneously
			var count = 0; // keep track of how many requests are ongoing simultaneously
                    ...
                    do_import = function( import_images ) {
	                    $.ajax({
                            ...
                            success: function( response ) {
								import_assets( response, function() {
									hideAlert();
